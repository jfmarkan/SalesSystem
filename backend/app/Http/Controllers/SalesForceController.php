...
class SalesforceController extends Controller
{
    ...
    /**
     * GET /api/sales-force/users (o similar)
     * Lista básica de usuarios con foto y team_id (1,2,3)
     */
    public function index(Request $request)
    {
        $hasRoe  = Schema::hasColumn('users', 'roe_id');
        $hasRole = Schema::hasColumn('users', 'role_id');
        $hasTeam = Schema::hasColumn('users', 'team_id');

        $selectCols = ['users.id','users.first_name','users.last_name'];
        if ($hasTeam) {
            $selectCols[] = 'users.team_id';
        }
        ...
        $users = $q->get()->map(function($u) use ($hasTeam) {
            return [
                'id'              => (int)$u->id,
                ...
                // team_id plano (1,2,3) o null si no está
                'team_id'         => $hasTeam && $u->team_id !== null ? (int)$u->team_id : null,
            ];
        });
     ...
     * (no es el endpoint de la vista que estamos trabajando,
     *   pero incluimos team_id aquí también por consistencia)
     */
    public function summary(Request $req)
    {
        ...
        // Subconsulta de usuarios para el join final (incluyendo team_id y avatar)
        $usersSubForJoin = DB::table('users as u')
            ...
            ")
            ->groupBy('u.id','u.first_name','u.last_name','u.team_id', ...($hasUserDetails ? ['ud.avatar_url'] : []))

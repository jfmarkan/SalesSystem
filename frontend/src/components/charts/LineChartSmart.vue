<script setup>
/* Smart line chart con loader interno.
   type: 'cumulative' | 'versions'  (alias 'cum','cummulative' -> 'cumulative') */
...
function mapVersions(d, budgetValue = null){
  ...
  if (m.length && arr.length && typeof arr[0] === 'object') {
    ...
    const base = arr.map((v, i)=>({
      ...
      tension:.25, pointRadius:0, fill:false
    }))
    if (budgetValue != null && !Number.isNaN(Number(budgetValue))) {
      base.push({
        label: 'Budget (Monat)',
...
async function fetchCumulative(){
  if (!props.clientId || !props.profitCenterId) { labels.value=[]; datasets.value=[]; return }
  loading.value = true
  try{
    await ensureCsrf()
...
watch(() => [props.type, props.clientId, props.profitCenterId, props.cumData, props.versionsData, props.autoFetch, props.busy], refresh, { deep:true })
onMounted(refresh)

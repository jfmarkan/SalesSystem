	...
	<div class="cal ">
		...
		<div class="cal-grid">
			...
			<template v-for="(week, wi) in weeks" :key="wi">
				<div v-for="cell in week" :key="cell.iso" class="cell"
					:class="{ off: !cell.inMonth, today: cell.isToday }" @click="openDay(cell.iso)">
					<span class="day">{{ dayNum(cell.date) }}</span>

					<div class="pill-stack">
						<template v-for="ev in dayEvents(cell.iso).slice(0, maxPills)" :key="ev.uid">
		...
		<div v-if="loading" class="overlay-loading">
			<div class="dots">
				<span v-for="i in 5" :key="i" :style="{ animationDelay: (i - 1) * 0.08 + 's' }"></span>
			</div>
				...
				<div v-for="ev in agenda" :key="ev.uid" class="row">
					...
					<div class="info">
						<div class="t1">
							...
							<span v-if="ev.type === 'action'" class="state-chip" :class="{ ok: ev.completed }">
								{{ ev.completed ? 'Erledigt' : 'Offen' }}
							</span>
						</div>

						<div class="t2">
							<span v-if="ev.start" class="chip"><i class="pi pi-clock" /> {{ ev.start
							}}<span v-if="ev.end">–{{ ev.end }}</span></span>
							<span v-if="ev.meta" class="chip"><i class="pi pi-info-circle" /> {{ ev.meta }}</span>
						</div>
						...
						<div v-if="ev.type === 'action'" class="editor">
							<label class="editor-field">
								<span>Fällig am</span>
								<input type="date" :value="ev.date" :disabled="saving.has(ev.uid)"
									@change="onChangeDueDate(ev, $event.target.value)" />
							</label>

							<button class="btn primary" :disabled="saving.has(ev.uid)"
								@click.stop="onToggleCompleted(ev)">
								<i class="pi" :class="ev.completed ? 'pi-undo' : 'pi-check-circle'"></i>
								<span>{{
...
.badge--action .pi.a-gray,
.ic.badge--action .pi.a-gray {
	color: #94a3b8;
}

.badge--action .pi.a-green,
.ic.badge--action .pi.a-green {
	color: #22c55e;
}

.badge--action .pi.a-yellow,
.ic.badge--action .pi.a-yellow {
	color: #f59e0b;
}

.badge--action .pi.a-red,
.ic.badge--action .pi.a-red {
	color: #ef4444;
}

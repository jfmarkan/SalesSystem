<template>
	<div class="panel">
		...
		<div class="actions">
			<Button
				:disabled="!canRun || loading || !canSimulate || skipBudget"
				label="Simulieren"
				icon="pi pi-play"
				@click="simulate"
			/>
		...
		<template v-else>
			<div v-if="summary" class="flex">
				<div class="summary">
					<div class="kpi">
						<span class="k">Sales CYTLFM (Basis)</span>
						<span class="v">{{ fmt(summary.totalSalesYTD) }}</span>
					</div>
					<div class="kpi">
						<span class="k">Saison-Anteil CYTLFM</span>
						<span class="v">{{ fmt(summary.ytdPct) }} %</span>
					</div>
					<div class="kpi">
						<span class="k">Basis-Prognose</span>
						<span class="v">{{ fmt(summary.baseForecast) }}</span>
					</div>
					<div class="kpi">
						<span class="k">Best Case (gesamt)</span>
						<span class="v good">{{ fmt(summary.totalBest) }}</span>
					</div>
					<div class="kpi">
						<span class="k">Worst Case (gesamt)</span>
						<span class="v bad">{{ fmt(summary.totalWorst) }}</span>
					</div>
				</div>
				<div class="legend">
			...
			<div v-else class="placeholder">
				<em>Wähle Client & Profit Center und klicke „Simulieren“.</em>
			</div>
		</template>
...
watch(bestCase, () => {
	touchedBest.value = true
})

const canRun = computed(
	() =>
		Number.isFinite(props.clientGroupNumber) &&
		Number.isFinite(props.profitCenterCode),
)
const canSimulate = computed(
	() =>
		touchedBest.value &&
		bestCase.value !== null &&
		!Number.isNaN(Number(bestCase.value)),
)
...
const dirty = computed(
	() =>
		toNumSafe(bestCase.value) !== toNumSafe(origBest.value) ||
		toNumSafe(worstCase.value) !== toNumSafe(origWorst.value) ||
...
async function loadSavedCase() {
	...
	try {
		...
		if (row) {
			const b = toNumSafe(row.best_case ?? 0)
			const w = toNumSafe(row.worst_case ?? 0)
			const s = !!row.skip_budget
...
async function simulate() {
	...
	try {
		...
		summary.value = null
		error.value =
			e?.response?.data?.message || 'Fehler bei der Vorschau'
		toast.add({
			severity: 'error',
			summary: 'Fehler',
			detail: error.value,
...
watch([bestCase, worstCase, skipBudget], ([b, w, s]) => {
	emit('values-change', {
		best_case: toNumSafe(b),
		worst_case: toNumSafe(w),
		skip_budget: !!s,
	})
})
...
watch(
	() => [props.clientGroupNumber, props.profitCenterCode],
	loadSavedCase,
)

<template>
  <div ref="rootRef" class="pc-overview-grid">
...
/* flag inyectado desde el padre para saber si estamos exportando PDF */
const isPdfExporting = inject('isPdfExporting', ref(false))
...
async function fetchOverview() {
  ...
  const p = (async () => {
    try {
      ...
      const { data: resp } = await api.get('/api/analytics/pc/overview', {
        params,
      })

      // Si entre tanto cambiaron props, descartamos este resultado
      if (myToken !== fetchToken) return

      data.value = resp

      // dejá que Vue y Chart.js re-rendericen
      await nextTick()
      await nextTick()
    } finally {
...
watch(
  () => [props.profitCenterCode, props.fiscalYearStart, props.asOf, props.unit],
  () => {
    fetchOverview()
  },
)
...
/* ⚙️ Opciones Chart – base + computed para controlar animación solo en export */

const miniBarBaseOptions = {
...
const miniBarOptions = computed(() => {
  const opt = { ...miniBarBaseOptions }
...
const multiPieOptions = computed(() => {
  const opt = { ...multiPieBaseOptions }
  if (isPdfExporting.value) {
    opt.animation = false
  }
  return opt
...
/* 🔒 EXPOSICIÓN PARA EL PADRE – asegura render completo antes del PDF */
async function waitUntilReady() {
  // 1) Esperar a que el último fetch finalice
  if (loadingPromise) {
    await loadingPromise
  }

  // 2) Dejar que Vue procese cualquier cambio pendiente
  await nextTick()
  await nextTick()
  await new Promise(resolve => requestAnimationFrame(resolve))
  await new Promise(resolve => requestAnimationFrame(resolve))

  // 3) Asegurar que todos los canvas dentro de este componente tengan tamaño
  await new Promise(resolve => {
    const check = () => {
      ...
      if (!canvases.length) {
        // si no hay charts, nada que esperar
        resolve()
        return
      }

      const allOk = canvases.every(
  ...
  // 4) Pequeño colchón extra por seguridad
  await new Promise(resolve => setTimeout(resolve, 150))
}

defineExpose({ waitUntilReady })
</script>

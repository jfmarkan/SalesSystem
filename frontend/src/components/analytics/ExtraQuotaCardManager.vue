...
function normalizeMix(m) {
	...
	const arr = Array.isArray(m)
		? m
		: Object.entries(m).map(([label, amount]) => ({ label, amount }))
	return arr.map((s, i) => ({
		key: s.key ?? String(i),
...
const pc = computed(() => {
	...
	const open = Math.max(
		0,
		Number(d.open) != null ? Number(d.open) : Math.max(0, allocated - won - lost),
	)
	...
	<div class="xq-root">
		...
		<div v-if="baseSegs.length" class="xq-stack" aria-label="Zusammensetzung">
			<div
				...
				:style="{
					width:
						(totals.totalAssigned > 0 ? (s.amountM3 * 100) / totals.totalAssigned : 0) +
						'%',
					background: s.color,
				}"
			...
			<div
				...
				:style="{
					left:
						Math.min(
							100,
							(totals.totalUsed / Math.max(1, totals.totalAssigned)) * 100,
						) + '%',
				}"
				title="Alokation"
			></div>
		...
		<transition name="fade">
			<div v-if="expanded && pc" class="pc-detail">
				...
				<div class="pc-legend">
					<span class="pitem"
						><i class="dot dot-win"></i> Gewonnen: {{ fmt(pc.won) }}
						{{ unitLabel() }}</span
					>
					<span class="pitem"
						><i class="dot dot-open"></i> Offen: {{ fmt(pc.open) }}
						{{ unitLabel() }}</span
					>
					<span class="pitem"
						><i class="dot dot-lost"></i> Verloren: {{ fmt(pc.lost) }}
						{{ unitLabel() }}</span
					>
					<span class="pitem sep"></span>
					<span class="pitem total"
						>Zugewiesen: {{ fmt(pc.allocated) }} {{ unitLabel() }}</span
					>
				</div>
			</div>
		</transition>

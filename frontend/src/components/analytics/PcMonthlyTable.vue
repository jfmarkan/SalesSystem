...
function pctRelLabel(num, den){
  const n = toNumberDE(num), d = toNumberDE(den)
  if (!d) return '0%'
...
function clsIstVsFcst(v, f){
  const n = toNumberDE(v), d = toNumberDE(f)
...
const salesData = computed(() => {
  const base = Array.isArray(props.sales) && props.sales.length ? props.sales
             : (Array.isArray(props.ventas) ? props.ventas : [])
    ...
    <div class="table-scroll-x">
      <table class="w-full" style="min-width: 1200px; border-collapse: separate; border-spacing: 0">
        ...
        <tbody>
          ...
          <tr>
            <td class="p-2 sticky text-right left-0 z-2 stick-left">% Ist</td>
            <td v-for="(m, i) in months" :key="'ivf' + i"
                class="p-2 text-center cell dev-cell"
                :class="[ clsIstVsFcst(salesData[i], forecastData[i]), { 'cur-left': i === curIdx, 'cur-right': i === curIdx } ]">

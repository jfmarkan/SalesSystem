...
function cancelCreateAndReset() {
  ...
  enterCreateMode()
  toast.add({ severity: 'warn', summary: 'Abgebrochen', detail: 'Dies ist Forecast. Vorgang abgebrochen.', life: 1800 })
}
...
async function saveNewVersion() {
  ...
  if (mustRebuild) { await rebuildBudgetFromForm(); scaleForecastByRatio() }
...
async function finalizeLost() {
  ...
  try {
    ...
    await ensureCsrf()
    await api.post(`/api/extra-quota/opportunities/${selectedGroupId.value}/${selectedVersion.value}/finalize`, { status: 'lost' })
    ...
    const msg = e?.response?.data?.message || e?.message || 'Fehler beim Finalisieren (verloren)'
    toast.add({ severity: 'error', summary: 'Fehler', detail: msg, life: 2200 })
  }
}

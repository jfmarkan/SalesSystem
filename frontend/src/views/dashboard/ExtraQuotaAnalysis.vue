...
function pct(num, den) {
    const d = den > 0 ? den : 1
    return Math.max(0, Math.min(100, Math.round(((num || 0) * 100) / d)))
}
...
async function load() {
    ...
    errorMsg.value = ''
    try {
        const { data } = await api.get('/api/extra-quota/analysis/summary', {
            params: { fiscal_year: fyStart.value },
        })
...
<style scoped>
/* ✅ Define palette globally so Chrome always sees the custom properties */
:global(:root) {
    --eq-green: #16a34a; /* verde */
    --eq-yellow: #f59e0b; /* amarillo/naranja */
    --eq-blue: #3b82f6; /* azul */
    --eq-red: #ef4444; /* rojo */
    --eq-gray: #64748b; /* gris */
    --eq-purple: #7c3aed; /* opcional */
}
...
/* Stacked bar: won (verde) + open ponderado (amarillo) */
.bar-row {
    display: grid;
...
/* 🔧 Icon colors (use fallbacks to be bulletproof in Chrome) */
.pi.ico-quote {

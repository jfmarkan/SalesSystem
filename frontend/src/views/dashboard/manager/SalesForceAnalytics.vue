            ...
            <div class="glass card-shadow panel tall-vh">
              <div class="panel-head">
                ...
                <div class="kpis flex align-items-center gap-2">
                  <Tag severity="success" value="Fristgerecht" rounded /><strong>{{ kpiInTerm }}</strong>
                  <span class="sep">|</span>
                  <Tag severity="warning" value="Verspätet" rounded /><strong>{{ kpiOutTerm }}</strong>
                </div>
              </div>
              ...
              >
                ...
                <Column header="Plan" style="width: 130px">
                  <template #body="{ data: r }">
                    <span v-if="hasPlan(r)" class="p-tag p-tag-success p-tag-rounded">Plan</span>
                    <span v-else class="p-tag p-tag-secondary p-tag-rounded">Kein Plan</span>
                  </template>
                </Column>

                <Column header="Status" style="width: 160px">
                  <template #body="{ data: r }">
                    <span v-if="isOverdue(r)" class="p-tag p-tag-danger p-tag-rounded">Überfällig</span>
                    <span
                      v-else
    ...
    >
      <template v-if="activeDeviation">
        <div class="dev-header">
          ...
          <div>
            <span v-if="isOverdue(activeDeviation)" class="p-tag p-tag-danger p-tag-rounded"
              >Überfällig</span
            >
            <span
              v-else
        ...
        <div class="grid gap-3">
          <div class="col-12 md:col-6">
            <div class="inner glass card-shadow">
              ...
              <ul class="kv">
                <li>
                  <span>Umsatz</span><strong>{{ fmtCurrency(activeDeviation.sales) }}</strong>
                </li>
                <li>
                  <span>Budget</span><strong>{{ fmtCurrency(activeDeviation.budget) }}</strong>
                </li>
                <li>
                  <span>Forecast</span><strong>{{ fmtCurrency(activeDeviation.forecast) }}</strong>
                </li>
                <li>
                  <span>Delta</span>
                  <strong :class="(activeDeviation.deltaAbs ?? 0) >= 0 ? 'pos' : 'neg'">
                ...
                <li>
                  ...
                  <strong
                    ...
                    ><span v-else>{{ activeDeviation.justified ? 'Begründet' : 'Offen' }}</span>
                  </strong>
                </li>
              </ul>
            </div>
          ...
          <div class="col-12 md:col-6">
            <div class="inner glass card-shadow">
              ...
              <p class="mono" v-if="activeDeviation.comment && activeDeviation.comment.trim().length">
                {{ activeDeviation.comment }}
              </p>
              <p v-else class="muted">Keine Begründung angegeben.</p>
              <div class="muted small" v-if="activeDeviation.justAuthor || activeDeviation.justDate">
                <i class="pi pi-user mr-1" v-if="activeDeviation.justAuthor" />{{
                  activeDeviation.justAuthor || ''
                }}
                <span v-if="activeDeviation.justAuthor && activeDeviation.justDate" class="mx-2"
                  >·</span
                >
          ...
          <div class="col-12">
            <div class="inner glass card-shadow">
              ...
              <p
                v-if="activeDeviation.plan && String(activeDeviation.plan).trim().length"
                class="mono"
              >
                <i class="pi pi-flag mr-2" />{{ activeDeviation.plan }}
              </p>
              <template v-if="Array.isArray(activeDeviation.actions) && activeDeviation.actions.length">
                <ul class="actions">
                  <li v-for="(a, i) in activeDeviation.actions" :key="i">
                    <i :class="['pi', a.done ? 'pi-check-circle text-success' : 'pi-circle']" />
                    <span class="ml-2">{{ a.title || '—' }}</span>
                    <span class="muted ml-2" v-if="a.due">· Fällig: {{ fmtDate(a.due) }}</span>
...
/* ========= Units by PC for Delta ========= */
const unitsByPc = ref({}) // { pcCode: 'VKEH' | 'M3' | 'EUR' | ... }
...
async function loadUnitsForDeviations() {
  const codes = Array.from(
    new Set((Array.isArray(deviations.value) ? deviations.value : [])
      .map(r => String(r?.pcCode || ''))
      .filter(Boolean))
  )
  if (!codes.length) { unitsByPc.value = {}; return }
  try {
    const { data } = await api.get('/api/unit-conversions/by-pc', { params: { codes: codes.join(',') } })
    unitsByPc.value = data || {}
  } catch {
    const m = {}; for (const c of codes) m[c] = 'VKEH'
    unitsByPc.value = m
  }
}
...
function initials(n = '') {
  return n.split(' ').filter(Boolean).map(w => w[0]?.toUpperCase()).slice(0, 2).join('')
}
...
function isOverdue(row) {
  const j = !!row?.justified, now = new Date(), y = now.getFullYear(), m = now.getMonth() + 1, day = now.getDate()
...
function displayNameFromApiUser(u = {}) {
  ...
  const last = p[p.length - 1], first = p.slice(0, -1).join(' ')
  return `${last}, ${first}`
}
const sellerItems = computed(() => {
  ...
  arr.sort((a, b) =>
    String(a.__displayName).localeCompare(String(b.__displayName), 'de', { sensitivity: 'base' }),
  )
  return arr
})
...
async function loadDeviationsForMonth() {
  ...
  const year = start.getFullYear(), month = start.getMonth() + 1

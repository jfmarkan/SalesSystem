          ...
          <div v-else-if="item.type === 'title'" class="h-full p-3 flex align-items-center justify-content-between">
            <div class="flex align-items-center gap-2">
              <ForecastTitle
                ...
                :pc="selectedPCName"
              />
              <span v-if="hasSelection && hasCaseForSelection" class="pill-ok" title="Budget Case vorhanden">
                ✓ Budget Case
              </span>
            </div>
...
function normalizeMode(v){ if(!v) return ''; const s=String(v).toLowerCase().trim(); if(['client','cliente','kunde'].includes(s)) return 'client'; if(['pc','profit','profitcenter','profit center'].includes(s)) return 'pc'; return '' }
function toNumberSafe(...vals){ for(const v of vals){ const n=Number(v); if(Number.isFinite(n)) return n } return null }
function fiscalYearFromDate(d){ const m=d.getMonth()+1, y=d.getFullYear(); return m<4? y-1 : y }
function nextFiscalYear(){ return fiscalYearFromDate(new Date()) + 1 }
...
function caseKey(cgn,pcc){ return `${Number(cgn)}-${Number(pcc)}` }
...
const secondaryOptions = computed(() => {
  ...
  if (mode.value === 'client') {
    ...
    const cgn = toNumberSafe(c?.client_group_number, c?.group_number, c?.clientGroupNumber, c?.client_group)
    return ids.map(id => {
  ...
  try{
    ...
    if (mode.value === 'client'){
      ...
      await Promise.all(pcIds.map(id=>{
        ...
        return api.get('/api/budget-cases', { params:{ client_group_number:cgn, profit_center_code:pcc, fiscal_year:fy } })
          .then(({data})=>{ if(data?.data) hasCaseSet.value.add(caseKey(cgn,pcc)) })
          .catch(()=>{})
      }).filter(Boolean))
    } else {
      const p = pcById.value[primaryId.value]
      ...
      await Promise.all(clIds.map(id=>{
        const c = clientById.value[id]; if(!c) return null
        const cgn = toNumberSafe(c?.client_group_number, c?.group_number, c?.clientGroupNumber, c?.client_group)
        if (cgn==null || pcc==null) return null
        return api.get('/api/budget-cases', { params:{ client_group_number:cgn, profit_center_code:pcc, fiscal_year:fy } })
...
const hasCaseForSelection = computed(() => {
  ...
  const cgn = cgnForChild.value, pcc = pccForChild.value
...
function onChildValues({ best_case, worst_case }){ bestLatest.value=Number(best_case)||0; worstLatest.value=Number(worst_case)||0; budgetDirty.value = true }
...
function applyChange(kind, value){
  if (kind==='mode'){ mode.value=value; primaryId.value=null; secondaryId.value=null; clearAll(); hasCaseSet.value=new Set() }
  else if (kind==='primary'){ primaryId.value=value; secondaryId.value=null; clearAll(); hasCaseSet.value=new Set(); refreshCaseFlagsForSecondary() }
  else if (kind==='secondary'){ secondaryId.value=value; clearAll() }
}
...
watch([mode, primaryId], () => { refreshCaseFlagsForSecondary() })
...
/* Pill verde */
.pill-ok{
  background:#e8f7ee; color:#0e7a3e; border:1px solid #b6e2c8;
  padding:2px 8px; border-radius:999px; font-size:.85rem; line-height:1.4;
}
</style>

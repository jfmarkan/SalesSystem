        ...
        <GlassCard :class="{ 'no-strip': item.type==='title' }" :title="getTitle(item)">
          ...
          <div v-else-if="item.type==='title'" class="h-full p-3 flex align-items-center justify-content-between">
            ...
            <div v-if="hasSelection" class="flex gap-2">
              <Button label="Speichern" icon="pi pi-save" :disabled="!budgetDirty" @click="saveBudgetCase" />
            </div>
          </div>
...
function toNumberSafe(...vals){ for (const v of vals){ const n=Number(v); if(Number.isFinite(n)) return n } return null }
...
async function loadMaster(){
  try{
    ...
    const cMap = {}; for (const c of clients.value) cMap[c.id] = c; clientById.value = cMap
    const pMap = {}; for (const p of profitCenters.value) pMap[p.id] = p; pcById.value = pMap
  }catch{
...
const overlayBest  = ref([])  // cumulativos, mismo largo que months
const overlayWorst = ref([])

function toCum(arr){
  const out=[]; let s=0
  for (let i=0;i<arr.length;i++){ s+=Number(arr[i]||0); out.push(s) }
  return out
}

/* Construye overlays a partir del emit del hijo: { seriesTarget: [{month, best, worst}, ...12]} */
function onSimulated(payload){
  const t = Array.isArray(payload?.seriesTarget) ? payload.seriesTarget : []
  // mensualidades (12 -> best/worst)
  const bestM  = t.map(x => Number(x?.best  || 0))
  const worstM = t.map(x => Number(x?.worst || 0))
  const bestC  = toCum(bestM)
  const worstC = toCum(worstM)

  // Alinear sobre labels YYYY-MM (colocar los 12 últimos slots)
  const len = months.value?.length || 0
...
function applyChange(kind,value){
  if (kind==='mode'){ mode.value=value; primaryId.value=null; secondaryId.value=null; clearSeries() }
  else if (kind==='primary'){ primaryId.value=value; secondaryId.value=null }
  else if (kind==='secondary'){ secondaryId.value=value }
}
function guardedChange(kind,value){ if (hasUnsaved.value){ pendingChange.value={kind,value}; confirmVisible.value=true } else { applyChange(kind,value) } }
async function saveBudgetCase(){ if (!bcRef.value) return; await bcRef.value.save(); budgetDirty.value=false; toast.add({ severity:'success', summary:'Gespeichert', detail:'Budget-Fall gespeichert', life:2200 }) }
async function saveAndApply(){ try{ if (hasUnsaved.value) await saveBudgetCase() } finally { confirmVisible.value=false; if (pendingChange.value) applyChange(pendingChange.value.kind, pendingChange.value.value); pendingChange.value=null } }
function discardAndApply(){ if (bcRef.value) bcRef.value.reset?.(); budgetDirty.value=false; confirmVisible.value=false; if (pendingChange.value) applyChange(pendingChange.value.kind, pendingChange.value.value); pendingChange.value=null }
function handleNext(){ const list=secondaryOptions.value; if (!list?.length) return; const idx=list.findIndex(o=>o.value===secondaryId.value); const n=(idx>=0?(idx+1):0)%list.length; guardedChange('secondary', list[n].value) }
...
const currentCGN = computed(() => {
  ...
  const fromClient = toNumberSafe(c?.client_group_number, c?.group_number, c?.clientGroupNumber, c?.client_group)
